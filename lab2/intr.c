...
void print(void) {
  char far* screen = (char far*)MK_FP(0xB800, 0); // возвращает дальний указатель по заданному сегменту (сегмент видеадаптера, смещение)

  // Master mask
  print_byte(screen, inp(0x21)); // inp возвращает значение по заданному адресу
